...
export function DatePicker({ currentDate, variant = 'default' }: DatePickerProps) {
  ...
  const getAdjacentDate = (baseDate: string, deltaDay: number) => {
      const newDate = new Date(baseDate + 'T00:00:00Z');
      newDate.setUTCDate(newDate.getUTCDate() + deltaDay);
      return newDate.toISOString().slice(0, 10);
    };

  const navigateToNextDay = () => {
    navigateToDate(getAdjacentDate(currentDate, 1));
    setSelectedDate(getAdjacentDate(currentDate, 1));
  };
  ...
  const arrowButtonClass =
    ...
    "bg-black/40 hover:bg-black/60 backdrop-blur-xl " +
    "border border-white/20 hover:border-white/30 " +
    "transition-all text-zinc-300 hover:text-zinc-100 " +
      ...
      <div className="relative">
        ...
        {isOpen && (
          <>
            ...
            {/* Date picker popup */}
            {/* FIX AT DATOEN INDE I DATEPICKER IKKE SKIFTER TIL CURRENT NÅR MAN BRUGER ARROWS */}
